...
public class GamepadController : AbstractController
{
	...
	private EQuadrants[] lastQuadrants = new EQuadrants[2] { EQuadrants.None, EQuadrants.None };
	...
	public static GamepadController Instance
	{
		get
		{
			...
			_instance = new GamepadController();
			_instance.Name = "gamePadController";
			StaticObjectsManager.AddObject(_instance, false);
			return _instance;
		}
	}
	...
	public void SubscribeUIElement(Node gameObject)
	{
		if (lastJoystickCount > 0)
		{
			gameObject.Visible = false;
		}
		JoystickConnect += (count) =>
		{
			gameObject.Visible = false;
		};
		JoystickDisconnect += (count) =>
		{
			if (count == 0)
			{
	...
	private void Initialize(float offset, float[] quadrantRanges)
	{
		...
		{
			GD.PrintErr("Bad Quadrant Ranges!");
		}
	}
	...
	{
		int count = Input.GetConnectedJoypads().Count;
