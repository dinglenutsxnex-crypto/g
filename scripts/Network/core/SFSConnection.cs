...
{
	public partial class SFSConnection : NetworkEventListner
	{
		...
		public void CreateInstance()
		{
			...
			{
				GD.PrintErr("<ERROR> This can be called only after we disconnected form outside.");
				return;
			}
		...
		private void OnWarnLogMessage(BaseEvent evt)
		{
			...
			GD.PushWarning("[SFS2X Warning]" + text);
		}
		private void OnErrorLogMessage(BaseEvent evt)
		{
			string text = (string)evt.Params["message"];
			GD.PrintErr("[SFS2X Error]" + text);
		}
		...
		private void OnExtensionResponse(BaseEvent e)
		{
			...
			if (sFSProtocolResponse.Code == 0)
			{
				...
				{
					GD.PrintErr(string.Format("Error: Cannot parse proto for cmd: {0} Proto on Client or Server is very likely out of date. Or request should have [state_change] in sf3.proto\nOriginal Error: {1}", text, ex.Message));
...
private void onRequestTimeout(int requestID)
		{
			...
			if (requestData != null)
			{
				...
				{
					GD.PrintErr(string.Format("Request timeout cmd={0} id={1}", requestData.cmd, requestID.ToString()));
				}
		...
		private void LogEvent(string cmd, Type type, IMessage e)
		{
			...
			{
				GD.Print(string.Format("[Server Event] cmd:<b>{0}</b> type:{1} extensible:\n{2}", cmd, type.ToString(), e.ToDebugJSON()));
			}
		}
		public void LogRequest(string source, string cmd, string extraInfo, Type type, IMessage e)
		{
			if (!IsIgnoreingCMDForLogging(cmd))
			{
				GD.Print(string.Format("[{0}] cmd:<b>{1}</b> type:{2} {3} extensible:\n{4}", source, cmd, type.ToString(), extraInfo, e.ToDebugJSON()));
			}
		}
	}
}
