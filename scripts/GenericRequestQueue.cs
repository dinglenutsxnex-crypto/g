...
using Godot;
public partial class GenericRequestQueue
{
	...
	private void CancelRequestWithTimeout(GenericRequest request)
	{
		...
		{
			GD.PrintErr("CancelRequestWithTimeout called with not found. Request cmd:" + request.cmd);
		}
	}
	...
	private void SendNextRequestFromQueue()
	{
		...
		if (isRequestInProgress())
		{
			GD.PrintErr("Error: checking to send from queue while we have a current request");
		}
	...
	private void sendRequest(GenericRequest r)
	{
		if (isRequestInProgress())
		{
			GD.PrintErr("Error: can't send multiple requests at the same time");
			return;
		}
		currentRequest = r;
