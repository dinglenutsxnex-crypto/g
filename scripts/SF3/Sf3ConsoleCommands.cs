...
namespace SF3
{
	public class Sf3ConsoleCommands
	{
		...
		{
			defaultWidth = DisplayServer.WindowGetSize().X;
			defaultHeight = DisplayServer.WindowGetSize().Y;
		...
		private static string FrameSkipShift(string[] args)
		{
			return SafeCommandExecution(delegate
			{
				...
				GD.Print("Frame skip shift set to " + FrameSkipController.MaximumFrameShift);
				return string.Empty;
			});
		}
		...
		private static string SetTextureQuality(string[] args)
		{
			return SafeCommandExecution(delegate
			{
				...
				if (int.TryParse(args[0], out result))
				{
					return "Texture quality level now " + result;
				}
				return "Can't parse";
			});
		}
		...
		private static string SetResolution(string[] args)
		{
			return SafeCommandExecution(delegate
			{
				...
				{
					DisplayServer.WindowSetSize(new Vector2I(Mathf.RoundToInt((float)defaultWidth * result), Mathf.RoundToInt((float)defaultHeight * result)));
		...
		{
			foreach (Node3D p in GetAllNodesOfType<Node3D>())
			{
				p.QueueFree();
			}
		...
		{
			foreach (Node3D child in GetAllNodesOfType<Node3D>())
			{
				RigidBody3D rb = child as RigidBody3D;
				if (rb != null)
				{
					rb.QueueFree();
				}
		...
		private static string GetBooster(params string[] args)
		{
			return SafeCommandExecution(delegate
			{
				if (args.Length > 0)
				{
					...
					Currency currency = new Currency { CurrencyType = CurrencyType.Coin };
					foreach (KeyValuePair<int, ShopBooster> shopBooster in JS.Instance.ShopBoosters)
					{
						if (shopBooster.Value.ID == num)
						{
							...
							AddCoins((currency.Value * num2).ToString());
		...
		{
			NetworkConnection.Send("cheat_set_experience", realExp, delegate(NetworkEvent e)
			{
		...
		private static string GetAllItems(params string[] args)
		{
			...
			NetworkConnection.Send("cheat_set_all_items", result, delegate(NetworkEvent e)
			{
				if (e.success)
				{
					NetworkConnection.current.RestartConnection("allitems cheat", false);
					GD.Print("GetAllItems Done!");
				}
				else
				{
					GD.PrintErr("GetAllItems Failed");
				}
			});
			return string.Empty;
		}
		...
		public static string BlockInput(string[] args)
		{
			...
			NekkiConsolePanel.Instance.OnButton();
			BlockScreenForAWhile(result);
			return string.Format("Input is blocked for [{0}] seconds", result);
		}

		public static async void BlockScreenForAWhile(int seconds)
		{
			while (isActive)
			{
				await Engine.GetMainLoop().ToSignal(Engine.GetMainLoop(), "process_frame");
			}
			UIBlocker.Instance.BlockNGUI();
			LoadingIcon.Instance.EnableLoadingScreen();
			await Engine.GetMainLoop().ToSignal(Engine.GetMainLoop(), "process_frame");
			await Engine.GetMainLoop().CreateTimer(seconds).Timeout;
			UIBlocker.Instance.UnblockNGUI();
			LoadingIcon.Instance.DisableLoadingScreen();
		}

		private static List<T> GetAllNodesOfType<T>() where T : Node
		{
			List<T> result = new List<T>();
			foreach (Node child in Engine.GetMainLoop().GetChildren())
			{
		...
		private static void CollectNodesRecursive<T>(Node node, List<T> list) where T : Node
		{
			if (node is T t)
			{
				list.Add(t);
