...
namespace SF3.Moves
{
	public partial class Condition : IConditionEqual
	{
		...
		public const string CurrentAnimationPlayer = "CurrentAnimationPlayer";
		...
		public static void Init()
		{
			...
			dictionary.Add("CurrentAnimationPlayer", GetCurrentAnimationPlayer);
			...
			dictionary.Add("InMirrorAnimationPlayer", InMirrorAnimationPlayer);
		...
		private static object RuleVariable(List<object> objects)
		{
			...
			GD.PrintErr("RPN RuleVariable Name:" + text + " Variable:" + text2 + " not available");
			return 0;
		}
		private static bool IsCountArguments(List<object> args, int countargs)
		{
			...
			{
				GD.PrintErr("RPN Argument Functions not available count:" + args.Count + " countargs:" + countargs);
				return false;
			}
			return true;
		}
		...
		public static object GetMaterial(List<object> objects)
		{
			...
			else
			{
				if (!(obj is HitResult))
				{
					GD.PrintErr("Could not cast argument to StrikeData orHitResult ");
					return "none";
				}
		...
		public static object GetArmorMaterial(List<object> objects)
		{
			...
			if (!(obj is HitResult))
			{
				GD.PrintErr("Could not cast argument to HitResult");
				return "none";
			}
		...
		public static object GetVariable(List<object> objects)
		{
			...
			else
			{
				if (!text.Equals("Parent"))
				{
					...
					{
						GD.PushWarning("GetVariable from child is not implemented!");
						return null;
					}
					GD.PrintErr("GetVariable got invalid target argument: " + text);
					return null;
				}
		...
		public static object GetCurrentAnimationPlayer(List<object> objects)
		{
			Model modelState = GetModelState(objects);
			return modelState.modelAnimationPlayer.animationNames;
		}
		...
		{
			Model modelState = GetModelState(objects);
			return modelState.modelAnimationPlayer.animationKey;
		}
		...
		public static object InMirrorAnimationPlayer(List<object> objects)
		{
			Model modelState = GetModelState(objects);
		...
		private static void ParseCondition(ref IConditionEqual mainCondition, Mapping nodeValue)
		{
			...
			text2 = text2.Trim();
			if (text.Contains("CurrentAnimationPlayer"))
			{
				text2 = text2.ToLower();
			}
			else if (text2.Contains("CurrentAnimationPlayer"))
			{
				text = text.ToLower();
			}
		...
		public static CompareOperationType GetCompareOperationByName(string str)
		{
			...
			case "LessEqual":
				...
				break;
			default:
				GD.PrintErr(string.Format("No compare operation with name [{0}] found", str));
				break;
			}
			return result;
		}
