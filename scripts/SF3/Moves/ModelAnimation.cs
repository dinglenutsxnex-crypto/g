...
namespace SF3.Moves
{
	public class ModelAnimation
	{
		...
		private void PlayInStack()
		{
			...
			{
				GD.PrintErr(string.Format("The amount of active animations on model {0} has reached a critical point of {1}!!!", _animatedModel.GetName(), 5));
				GD.PrintErr("----Current playing:");
				foreach (ActiveAnimation playingAnimation in _playingAnimations)
				{
					GD.PrintErr(playingAnimation.modelInfoAnim.animation.name);
				}
				GD.PrintErr("----");
				return;
			}
