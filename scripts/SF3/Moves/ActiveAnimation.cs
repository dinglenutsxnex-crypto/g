...
namespace SF3.Moves
{
	public class ActiveAnimation
	{
		...
		private Quaternion quaternionForBackward = new Quaternion(Vector3.Up, Mathf.DegToRad(180f));
		...
		public void Stop(IAnimatedModel bonesHolder, ref Quaternion rootRotation)
		{
			if (modelInfoAnim != null)
			{
				...
				{
					rootRotation = Quaternion.FromEuler(_blendAnimatedTransforms.animatedTransforms[modelInfoAnim.rootID].rotation.GetEuler() - _offsetRotation);
				}
			}
		}
		...
		private void ApplyOffsets()
		{
			if (!modelInfoAnim.animation.physical)
			{
				...
				animatedTransform.SetPosition(position);
				animatedTransform.SetRotation(Quaternion.FromEuler(animatedTransform.rotation.GetEuler() + _offsetRotation));
			}
			else
			{
		...
		private void Interpolate(float weight)
		{
			...
			{
				GD.PrintErr(string.Format("Havnt [{0}] interpolation!!", modelInfoAnim.animation.interpolation));
			}
		}
		...
		{
			return new Quaternion(vec.Normalized(), vec.Length());
		}
