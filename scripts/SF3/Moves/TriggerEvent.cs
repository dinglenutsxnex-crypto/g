...
namespace SF3.Moves
{
	public class TriggerEvent : Parseable
	{
		...
		public static ETriggerEvents GetEventTypeByName(string name)
		{
			...
			GD.PrintErr("getEventTypeByName - unknown type: " + name);
		...
		public static List<TriggerEvent> Parse(Sequence nodes)
		{
			...
			foreach (Node item2 in nodes.nodesInside)
			{
				...
				{
					GD.PrintErr(string.Format("Cant find event with name {0}", item2));
				}
				...
				{
					GD.PrintErr(string.Format("Havn't any event initialization for {0}", eTriggerEvents));
				}
			}
			return list;
		}
