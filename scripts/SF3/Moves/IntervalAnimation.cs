...
{
	[Serializable]
	public partial class IntervalAnimationPlayer
	{
		...
		public IntervalAnimationPlayer(EIntervalsType typeInterval = EIntervalsType.INTERVAL_NONE)
		{
			name = string.Empty;
			type = typeInterval;
			start = 0;
		...
		static IntervalAnimationPlayer()
		{
		...
		public static List<IntervalAnimationPlayer> Create(Sequence intervalsNodes)
		{
			...
			foreach (Mapping item in intervalsNodes.nodesInside)
			{
				...
				IntervalAnimationPlayer intervalAnimationPlayer = ((text == null) ? new IntervalAnimationPlayer() : (text.text.Equals("Attack") ? new IntervalAttack() : ((!text.text.Equals("TurnOffRepulsionBones")) ? new IntervalAnimationPlayer() : new IntervalExcludeRepulsion())));
				List<IntervalAnimationPlayer> list2 = intervalAnimationPlayer.Parse(item);
				if (list2.Count > 0)
				{
					list.AddRange(list2);
				}
				...
				{
					GD.PushWarning("IntervalAnimationPlayer is null");
				}
			}
			return list;
		}
		public virtual List<IntervalAnimationPlayer> Parse(Mapping intervalNode)
		{
