...
namespace SF3.Moves
{
	public partial class TriggerActionAnimationRandom : TriggerAction
	{
			...
			: base(EActionType.ANIMATION_RANDOM, yamNode)
		{
			...
			_prevID = GD.RandRange(0, ArrayNames.Length);
			_repetition = 0;
		}
		protected override void ApplyAction(object modelData)
		{
			int num = NextRandom(ArrayNames.Length);
			((Model)modelData).modelAnimationPlayer.Play(ArrayNames[num]);
		}
		private int NextRandom(int range)
		{
			int num = GD.RandRange(0, range);
			if (num == _prevID)
			{
				_repetition++;
				if (_repetition > 0)
