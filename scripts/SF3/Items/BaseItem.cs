...
namespace SF3.Items
{
	public partial class BaseItem : ICloneable, UserBadgesManager.IBadgeItem
	{
		...
		protected string GetNodeSafe(Dictionary<string, JsValue> dic, string key, string defaultValue)
		{
			if (dic == null)
			{
				GD.PrintErr("Achtung! JSDictionary is null");
				return defaultValue;
			}
		...
		protected string GetNodeSafe(JsValue table, string key, string defaultValue)
		{
			if (null == table)
			{
				GD.PrintErr("Achtung! Not found : Key[" + key + "], returning default value --->" + defaultValue);
				return defaultValue;
			}
