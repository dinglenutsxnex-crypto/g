...
namespace SF3.Items
{
	public partial class InventoryManager : UIModuleHolder
	{
		[Export]
		...
		public Node compareItemsBtn;

		public Node equippedLbl;

		public Node equippedFrame;
		...
		public Node3D subTypeSelectionFrame;
		...
		public override void Initialize()
		{
			...
			offsetFromChar.X *= (float)DisplayServer.WindowGetSize().X / (float)DisplayServer.WindowGetSize().Y / 1.33f;
			subTypeSelectionFrame.Visible = false;
			...
			perkInfoBarPosition.AddChild(perkInfoBarNode);
			if (perkInfoBarNode is Node3D n3d)
			{
				n3d.Scale = Vector3.One;
				n3d.Position = Vector3.Zero;
			}
			_perkInfroDrawer = perkInfoBarNode as PerkInfoDrawer ?? perkInfoBarNode.GetNodeOrNull<PerkInfoDrawer>(".");
			AudioManager.Instance.LoadSound(_onOpenSoundName, _onCloseSoundName);
			base.Node.SetActive(false);
		}
		...
		public static void EnableReelCameraIfExists()
		{
			...
			{
				Instance.inventoryReelDriver.EnableReelCamera3D();
			}
		}
		...
		public static void DisableReelCameraIfExists()
		{
			if (!(Instance == null) && !(Instance.inventoryReelDriver == null))
			{
				Instance.inventoryReelDriver.DisableReelCamera3D();
			}
		}
		...
		private void UpdateAbilitiesBadge(BaseItem item)
		{
			...
			if (item != null && item is Equipment)
			{
				...
				_abilitiesBadge.Node.SetActive(_abilitiesBadgeCount > 0);
		...
		private void ShowPerkDescription(BaseItem selectedBase)
		{
			...
			perkNameLbl.Node.SetActive(flag);
			perkDescriptionLbl.Node.SetActive(flag);
			NoPerksLabel.Node.SetActive(!flag);
			if (flag)
			{
		...
		public override void ShowModule(IntentModule intent, Action callbackOnOpen)
		{
			...
			{
				BattleCamera3D.Instance.changeClipPlane = true;
				BattleCamera3D.Instance.toClipPlane = 10f;
			}
			else
			{
				BattleCamera3D.Instance.MainCamera3D.nearClipPlane = 10f;
			}
			if (ModelsManager.Instance.Player != null)
			{
				BattleCamera3D.MoveToObject(_playerModel.GetCenterOfMassBone().Node3D, offsetFromChar, delegate
				{
					BattleCamera3D.ActivateInventoryCamera3D(offsetFromChar);
		...
		public override void HideModule(Action callbackOnClosed)
		{
			...
			_lastClickedSlot = null;
			inventoryReelDriver.Node.SetActive(false);
			inventoryReelBack.SetActive(false);
		...
		private void ShowPerkScreen()
		{
			...
			_abilitiesBadge.Node.SetActive(false);
			inventory.SetActive(false);
		...
		private void SelectSubtypeButton(EquipmentType type)
		{
			...
			{
				_lastClickedSlot = subtypeButton.subTypeButton.Node;
		...
		public void UpdateSlot(InventorySlot currentSlot)
		{
			...
			currentSlot.CategoryAlert.Node.SetActive(false);
			...
			foreach (Equipment item in equipmentOfType)
			{
				...
				{
					currentSlot.CategoryAlert.Node.SetActive(true);
					break;
				}
			}
		}
