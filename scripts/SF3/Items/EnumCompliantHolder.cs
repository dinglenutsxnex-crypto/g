...
namespace SF3.Items
{
	public abstract class EnumCompliantHolder<T> where T : struct
	{
		...
		protected T GetValueByKey(string key)
		{
			...
			GD.PrintErr("No value was found by key: [" + key + "]. Default value returned.");
		...
		protected string GetKeyByType(T type)
		{
			...
			List<string> list = _typeCompliance.Keys.Where(key => key.Equals(keyToSearch)).ToList();
			int count = list.Count;
			if (count == 1)
			{
				return list[0];
			}
			GD.PrintErr("No key was found by key: [" + keyToSearch + "]. Empty string returned.");
			return string.Empty;
		}
