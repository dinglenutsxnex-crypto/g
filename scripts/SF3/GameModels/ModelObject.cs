...
namespace SF3.GameModels
{
	public class ModelObject : IBonesHolder
	{
		...
		public void LoadModel(Equipment equipmentModel, Node3D parent, Gender genderValue, bool destroyPreviousModel = true)
		{
		...
		public virtual void LoadModel(string modelName, Node3D parent, Gender genderValue, bool destroyPreviousModel = true)
		{
		...
		public void DestroyItem()
		{
			...
			for (int j = 0; j < skeletons.Count; j++)
			{
				if (skeletons[j] != null)
				{
					skeletons[j].QueueFree();
				}
			}
		}

		private void CreateModelBones(Node3D parent, Gender genderValue)
		{
			...
			foreach (XmlElement item in xmlNodeList)
			{
				...
				if (attribute.Length == 0)
				{
					...
					Node3D gameObject = new Node3D();
					gameObject.Name = attribute2;
					skeletonObject = new SkeletonObject();
					gameObject.AddChild(skeletonObject);
				...
				{
					Node3D gameObject = LoadBonesPrefab(attribute) as Node3D;
					gameObject.Name = gameObject.Name.Replace("Models^Bones^Prefabs^", string.Empty);
					skeletonObject = gameObject as SkeletonObject;
					...
					Vector3 localPosition = gameObject.Position;
					Quaternion localRotation = gameObject.Quaternion;
					Vector3 localScale = gameObject.Scale;
					parent.AddChild(gameObject);
					gameObject.Position = localPosition;
					gameObject.Quaternion = localRotation;
				...
				skeletonObject.CreateSkeleton(skeletonObject, bonesDataNode, attribute3);
		...
		public void Init(IBonesHolder modelState, Node3D parent)
		{
			foreach (SkeletonObject skeleton in skeletons)
			{
		...
		private void CreateModelSkins(Node3D parent, IBonesHolder modelState = null)
		{
			if (_modelObjectNode == null)
			{
				return;
			...
			foreach (XmlElement item in xmlNodeList)
			{
				...
				Node3D gameObject = LoadSkinPrefab(value) as Node3D;
				if (gameObject == null)
				{
				...
				parent.AddChild(componentInChildren);
				if (modelState == null || modelState.GetBonesCount() == 0)
				{
					...
					{
						GD.PrintErr(string.Format("Cant bound \"{0}\" skin, model havnt bones ^^", name));
					}
				}
				else
				{
		...
		{
			InteractiveModelObject interactiveModelObject = new InteractiveModelObject();
			interactiveModelObject.Name = "Interactive_" + name;
			ResetBonesPosition();
			Node3D transform = interactiveModelObject as Node3D;
			FightSettings.DisarmTrajectory disarmTrajectory = (FightSettings.DisarmTrajectory)FightSettings.GetParamsByName("Disarm");
			Vector3 vector = vector.Lerp(disarmTrajectory.directionMin, disarmTrajectory.directionMax, GD.Randf());
			vector.X *= Mathf.Sign(attackImpulse.X);
			Vector3 vector2 = vector.Lerp(disarmTrajectory.torqueMin, disarmTrajectory.torqueMax, GD.Randf());
			float num = GD.RandRange(disarmTrajectory.impulseMin, disarmTrajectory.impulseMax);
			int num2 = 0;
			...
			foreach (SkeletonObject skeleton in skeletons)
			{
				...
				{
					skeleton.bones[0].GetParent().RemoveChild(skeleton.bones[0]);
					transform.AddChild(skeleton.bones[0]);
					if (!freeze)
					{
			...
			foreach (ModelSkin skin in skins)
			{
				if (skin.droppable)
				{
					skin.GetParent().RemoveChild(skin);
					transform.AddChild(skin);
				}
			}
			switch (sfState)
			{
		...
		public void SetMainColor(Godot.Color color, bool useAlpha = true)
		{
			foreach (ModelSkin skin in skins)
			{
		...
		public void SetSkinColor(Godot.Color color)
		{
			foreach (ModelSkin skin in skins)
			{
		...
		{
			// STUB: PreRenderManager not ported
		}
		...
		public Bone GetBone(string nameBone)
		{
			...
			GD.PrintErr(string.Format("Havnt any bone with name [{0}]", nameBone));
			return null;
		}
		...
		public Bone GetBone(int idBone)
		{
			...
			GD.PrintErr(string.Format("Havnt any bone with id [{0}]", idBone));
			return null;
		}
		...
		public static Node3D LoadBonesPrefab(string namePrefab)
		{
			return GlobalLoad.GetPrefabInstanceInternal("models_folder", "Bones/Prefabs/" + namePrefab) as Node3D;
		}
		...
		public static Node3D LoadSkinPrefab(string namePrefab)
		{
			...
			return GlobalLoad.GetPrefabInstanceInternal("models_folder", "Skins/Meshes/" + text) as Node3D;
		}
	}
}
