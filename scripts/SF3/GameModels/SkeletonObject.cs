...
{
	[Serializable]
	public partial class SkeletonObject : ExtentionBehaviour, IBonesHolder
	{
		...
		public void CreateSkeleton(Node3D parentBoneNode3D, XmlElement bonesDataNode, string mirroringDuplicateIndex)
		{
			...
			bones = new Bone[0];
			floorHitTransforms = new List<Node3D>();
			CreateBones(parentBoneNode3D);
			SetupRagdoll component = base.GetComponent<SetupRagdoll>();
			if (component != null)
			{
				component.Initialize(this);
			}
			skeletonRagdoll = base.GetComponent<SkeletonRagdoll>();
		...
		private void CreateRepulsionRect(XmlElement rectRepulsionNode)
		{
			...
			foreach (XmlElement childNode in rectRepulsionNode.ChildNodes)
			{
				...
				dictionary.Add(attributeNode.Value, GetBone(attributeNode.Value).Node3D);
			}
		...
		private void CreateBones(Node3D parentBoneNode3D)
		{
			...
			CreateBonesHierarchy(parentBoneNode3D, dictionary);
			foreach (string item in list)
			{
				floorHitTransforms.Add(GetBone(int.Parse(item)).Node3D);
			}
		}
		...
		private void CreateBonesHierarchy(Node3D parentBoneNode3D, Dictionary<string, BoneData> bonesIDData)
		{
			...
			if (additionalPart)
			{
				Node3D = parentBoneNode3D;
			}
			else
			{
				Node3D[] componentsInChildren = parentBoneNode3D.GetComponentsInChildren<Node3D>();
				Node3D[] array = componentsInChildren;
				foreach (Node3D transform2 in array)
				{
			...
			if (Node3D == null)
			{
				throw new Exception(string.Format("Cant find root for create bones hierarchy in Node3D [{0}]", parentBoneNode3D.name));
			}
			bones = new Bone[Node3D.GetComponentsInChildren<Node3D>().Length];
			Bone bone = new Bone(Node3D);
			...
			bones[num] = bone;
			num++;
			CreateBonesHierarchyRecursively(bone, Node3D, bonesIDData, ref num);
		}
		private void CreateBonesHierarchyRecursively(Bone parentBone, Node3D parentTransf, Dictionary<string, BoneData> bonesIDData, ref int num)
		{
			foreach (Node3D item in parentTransf)
			{
				Node3D transform2 = item;
		...
		private void InitVertexes(IBonesHolder modelState, XmlElement vertexesNode)
		{
			...
			foreach (XmlElement childNode in vertexesNode.ChildNodes)
			{
				...
				{
					GD.Print(string.Format("Vertex with name - {0} does not exist!!", childNode.GetAttribute("BoneName")));
				}
			}
		}
		private void InitCapsules(XmlElement capsulesNode)
		{
			...
			foreach (XmlElement childNode in capsulesNode.ChildNodes)
			{
				...
				{
					Capsule capsule = new Capsule(vertexByName, vertexByName2, float.Parse(childNode.GetAttribute("Radius"), CultureInfo.InvariantCulture), childNode.GetAttribute("Name"), childNode.GetAttribute("ParentRigidBody3D"));
