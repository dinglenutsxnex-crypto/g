...
{
	public partial class Model : Node, IEventSender, IAnimatedModel, ISkinnedModel, IModelHUD, IShadowFormModel
	{
		...
		// STUB: Collider → Godot PhysicsBody3D/Area3D
		public Node CurrentWallCollider { get; private set; }
		...
		public void UpdateShadow()
		{
			...
			{
				Godot.Color color = ShadowMapController.Instance.CalculateShadowAtPoint(modelComponents.rootBone.Position.X);
		...
		public async void AnimateTransperent(float from, float to, float duration, Action animationEndCallback = null)
		{
			float startTime = (float)Engine.GetProcessTime();
			float endTime = startTime + duration;
			while ((float)Engine.GetProcessTime() <= endTime)
			{
				float timePassed = (float)Engine.GetProcessTime() - startTime;
				float currentAlpha = Godot.Mathf.Lerp(from, to, timePassed / duration);
		...
		{
			return false; // STUB: coroutine tracking removed
		}

		public void AnimateShadowColor(Godot.Color to, float duration, Action animationEndCallback = null)
		{
			// STUB: DOTween removed, needs Godot Tween
			GD.Print("STUB: Model.AnimateShadowColor - DOTween removed, needs Godot Tween");
		}
		...
		public void Fade(float duration = 1f, bool show = false)
		{
			...
			if (!show)
			{
				...
				AnimateShadowColor(Colors.White, duration);
			}
			else
			{
				AnimateTransperent(0f, 1f, duration);
				AnimateShadowColor(_modelShadow != null ? _modelShadow.SavedShadowColor : Colors.White, duration);
			}
		}
		...
		public Vector3 GetPositionOf(DistancePoint.EDistanceObject objectType, string objectName)
		{
			...
			case DistancePoint.EDistanceObject.OBJECT_CENTER:
				...
				break;
			default:
				GD.PrintErr("Unexpected object argument ^^");
				return Vector3.Zero;
			}
			return result;
		}
		...
		public void WallHit(WallConfig.EWallType wallType, Node wall)
		{
		...
		private void CreateModel(ModelInfo _modelInfo, int modelID = -1)
		{
			...
			Name = _modelInfo.alias;
			Position = Vector3.Zero;
			// STUB: AddChild(new ObjectMaterialsControll())
			statusControl = new ModelStatusControl(this);
			moveControl = new ModelMoveControl(this, this);
			modelComponents = new ModelComponents(modelInfo, this, this);
			modelComponents.SetTransparent(1f);
			// STUB: GetComponentInChildren<ModelShadow>() → GetNode
			_modelShadow = null; // GetNodeInChildren<ModelShadow>()
			if (_modelShadow != null)
			{
				_modelShadow.CreateShadow();
			}
			ragdolAlignment = GetNode<RagdolAlignment>("RagdolAlignment");
			if (ragdolAlignment == null)
			{
				ragdolAlignment = new RagdolAlignment();
				AddChild(ragdolAlignment);
			}
		...
		public static EPlayerType GetPlayerTypeByName(string typeName)
		{
			...
			GD.PrintErr("ModelType - parseType - unknownType: " + typeName);
		...
		public static EPivotObject GetPivotObjectByName(string pivotObjectName)
		{
			...
			GD.PrintErr("EPivotObject - parseType - unknownType: " + pivotObjectName);
