	...
	public class ModelInfo : ICloneable
	{
		...
		internal IEnumerable<sf3DTO.Booster> GetDTOBoosters()
		{
			...
			foreach (List<SF3.Items.Booster> value in GetBoostersAll().Values)
			{
				...
				{
					sf3DTO.Loot loot = new sf3DTO.Loot();
		...
		public void ResetLifeState()
		{
			...
			{
				currentLife = Godot.Mathf.Clamp(currentLife + maxLife * FightController.Instance.CurrentFight.hpRecovery, 0f, maxLife);
			}
			else
			{
				currentLife = maxLife;
		...
		public void ChangeLife(float delta)
		{
			...
			currentLife = Godot.Mathf.Max(Godot.Mathf.Min(a, maxLife), 0f);
			if (currentLife <= 0f)
			{
