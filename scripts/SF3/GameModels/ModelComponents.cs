...
namespace SF3.GameModels
{
	public class ModelComponents : IBonesHolder, IShadowFormModel
	{
		...
		private Node3D _parentTransform;
		...
		public ModelComponents(ModelInfo info, Model skinnedModelValue, Node3D parentTransform)
		{
			this.info = info;
			...
			Node3D comNode = new Node3D();
			comNode.Name = "centerOfMass";
			centerOfMassBone = new Bone(comNode);
			_parentTransform.AddChild(centerOfMassBone.transform);
			InitializeEquipment();
			modelPhysics = new ModelPhysics(parentTransform, this);
		}
		...
		private ModelObject CreateModelObject(Equipment itemValue)
		{
			...
			{
				GD.PrintErr(string.Format("Cant find config for model [{0}]", itemValue.model));
		...
		private ModelObject CreateModelObject(string itemModel)
		{
			...
			{
				GD.PrintErr(string.Format("Cant load model object with name [{0}]", itemModel));
			}
			return modelObject;
		}
		...
		public void SetHead(string head)
		{
			...
			{
				GD.Print(string.Format("Cant load model object with name [{0}]", head));
			}
			InitializeEquipment();
		}
			...
			foreach (KeyValuePair<EquipmentType, ModelObject> equippedItem in equippedItems)
			{
				...
				foreach (SkeletonObject skeleton in equippedItem.Value.skeletons)
				{
					if (skeleton.additionalPart)
					{
						...
						Vector3 localPosition = skeleton.Position;
						Quaternion localRotation = skeleton.Quaternion;
						Vector3 localScale = skeleton.Scale;
						...
						if (component == null || component.anchor == null)
						{
							...
							bone.transform.AddChild(skeleton);
							skeleton.Position = localPosition;
							skeleton.Quaternion = localRotation;
							skeleton.Scale = localScale;
						}
						else if (component != null && component.anchor != null)
						{
							skeleton.GetParent().RemoveChild(skeleton);
							component.anchor.transform.AddChild(skeleton);
							skeleton.Position = component.anchor.transform.Position;
							skeleton.Quaternion = component.anchor.transform.Quaternion;
							skeleton.Scale = component.anchor.transform.Scale;
						}
		...
		{
			// STUB: HingeJoint/FixedJoint/CharacterJoint → Godot GenericJoint3D or custom joint system
			foreach (ModelObject value in equippedItems.Values)
			{
				foreach (SkeletonObject skeleton in value.skeletons)
				{
					// HingeJoint → GenericJoint3D stub
					// FixedJoint → GenericJoint3D stub
					// CharacterJoint → GenericJoint3D stub
				}
			}
		}
		...
		public void CollectCapsules()
		{
			...
			foreach (ModelObject value in equippedItems.Values)
			{
				...
				foreach (SkeletonObject skeleton in value.skeletons)
				{
					...
					foreach (KeyValuePair<string, Node3D> point in skeleton.repulsionRect.points)
					{
			...
			foreach (ModelObject commonEquippedItem in commonEquippedItems)
			{
				...
				foreach (SkeletonObject skeleton2 in commonEquippedItem.skeletons)
				{
					...
					foreach (KeyValuePair<string, Node3D> point2 in skeleton2.repulsionRect.points)
					{
			...
			modelCapsules.SetFloorRepulsion(new List<Node3D> { rootBone.transform }, 1f, 1f);
		}
				...
				bool flag = true;
				{
					foreach (ModelMaterial item in capsulesMaterials[capsuleName])
					{
						...
						foreach (string vertexName in vertexNames)
						{
							...
							if (flag)
							{
								num2 = (point - position).LengthSquared();
								num = num2;
		...
		public Bone GetBone(string boneName)
		{
			...
			GD.PushWarning("missing bone  " + boneName);
			return null;
		}
		...
		public void ChangeSkinsColor(Godot.Color color, bool useAlpha = true)
		{
			foreach (ModelObject value in equippedItems.Values)
			{
		...
		private void InitColors()
		{
			...
			{
				GD.PushWarning("Missing color manager");
				return;
			}
		...
		private void SetHairColor(sf3DTO.Color colorData)
		{
			...
			if (hairColor != null)
			{
				Godot.Color color = hairColor.GetColor((float)colorData.Value);
				SetHairColor(color);
			}
			else
			{
				GD.PushWarning("Missing  hair color " + colorData.ColorId);
			}
		}

		public void SetHairColor(Godot.Color col)
		{
			foreach (KeyValuePair<EquipmentType, ModelObject> equippedItem in equippedItems)
			{
		...
		private void SetSkinColor(sf3DTO.Color colorData)
		{
			...
			if (skinColor != null)
			{
				Godot.Color color = skinColor.GetColor((float)colorData.Value);
				SetSkinColor(color);
			}
			else
			{
				GD.PushWarning("Missing color " + colorData.ColorId);
			}
		}

		public void SetSkinColor(Godot.Color col)
		{
			foreach (KeyValuePair<EquipmentType, ModelObject> equippedItem in equippedItems)
			{
				if (equippedItem.Key != EquipmentType.Helmet)
