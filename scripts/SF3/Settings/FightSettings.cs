		...
		public static void Init()
		{
			...
			if (sequence != null)
			{
				for (int i = 0; i < sequence.nodesInside.Count; i++)
				{
					...
					if (text == null)
					{
						GD.PrintErr("Cant parse event name!!!");
						continue;
					}
					Mapping mapping2 = (sequence.nodesInside[i] as Mapping).GetMapping("Params");
					if (mapping2 == null)
					{
						GD.PrintErr("Cant parse event parameters!!!");
						continue;
					}
			...
			if (text3 == null)
			{
				GD.PrintErr("Cant find default Skeleton value in FightSettings");
			}
			defaultSkeleton = text3.text;
			sequence = mapping.GetSequence("DefaultTags");
			if (sequence == null)
			{
				GD.PrintErr("Cant find default Tags values in FightSettings");
			}
