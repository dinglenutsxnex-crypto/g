...
namespace SF3.Tactics
{
    public class ResultsTableAnalyzer
    {
        ...
        public ResultsTableAnalyzer(List<string> animationNames)
        {
            ...
            _animationSet = new HashSet<int>(animationNames.FindAll(a => _complianceTable.ContainsKey(a)).ConvertAll(a => _complianceTable[a]));
            _idleSet = new HashSet<int>(animationNames.FindAll(a => _complianceTable.ContainsKey(a) && MovesController.GetAnimationByName(a).groupNames.Contains("stance_idle")).ConvertAll(a => _complianceTable[a]));
        }
        ...
        public int GetCompliance(string animationName)
        {
            ...
            GD.PrintWarning($"Animation witn name {animationName} not found. MB you have obsolete results tables");
            return -1;
        }
