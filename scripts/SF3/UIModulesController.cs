...
namespace SF3
{
	public class UIModulesController
	{
		...
		static UIModulesController()
		{
			...
			{
				{ EUIElements.FOE_HUD, "FoeHUD" },
				{ EUIElements.PLAYER_HUD, "PlayerHUD" },
				{ EUIElements.PAUSE_WINDOW, "PauseWindow" },
				{ EUIElements.PAUSE_BUTTON, "PauseButton" },
				{ EUIElements.ROUND_TIMER, "RoundTimer" },
				{ EUIElements.ROUNDS_UI, "RoundsUI" },
				{ EUIElements.CONSOLE, "Console" },
				{ EUIElements.HOME_MENU, "HomeMenu" },
				{ EUIElements.MODEL_DEBUG, "ModelDebug" },
				{ EUIElements.MODEL_DEBUG_ENEMY, "ModelDebugEnemy" },
				{ EUIElements.DEBUG_HIDER, "DebugHider" },
				{ EUIElements.GAME_TIME_DEBUG, "GameTimeDebug" },
				{ EUIElements.DEBUG_MENU, "DebugMenu" },
				{ EUIElements.CURRENCY, "Currency" },
				{ EUIElements.DEBUG, "Debug" },
				{ EUIElements.FIGHT_BUTTONS, "FightButtons" },
				{ EUIElements.JOYSTICK, "Joystick" },
				{ EUIElements.SHADOW_PERKS, "ShadowPerks" },
				{ EUIElements.UIBLOCKER, "UIBlocker" }
			};
			_fightUIModules = new EUIElements[7]
			{
				EUIElements.FOE_HUD, EUIElements.PLAYER_HUD, EUIElements.PAUSE_WINDOW,
				EUIElements.PAUSE_BUTTON, EUIElements.ROUND_TIMER, EUIElements.ROUNDS_UI,
				EUIElements.SHADOW_PERKS
			};
			_dojoUIModules = new EUIElements[2]
			{
				EUIElements.CURRENCY, EUIElements.HOME_MENU
			};
			_controlUIModules = new EUIElements[2]
			{
				EUIElements.JOYSTICK, EUIElements.FIGHT_BUTTONS
			};
		}
		...
		public UIModulesController()
		{
			...
			if (NekkiUtils.IsDebug)
			{
				...
				nekkiUIModule.Elements.GetButton("reset").Pressed += NekkiUtils.ClearAllApplication;
		...
		private List<NekkiUIModule> MountModules(EUIElements[] newModules, Vector3? scaleVector = null)
		{
			...
			foreach (EUIElements eUIElements in newModules)
			{
				...
				{
					nekkiUIModule.Scale = scaleVector.Value;
				}
			}
			return list;
		}
		...
		public NekkiUIModule GetMountedModule(EUIElements module)
		{
			...
			{
				if (item.ModuleName.Equals(value)) return item;
			}
			foreach (NekkiUIModule item2 in _mountedLocal)
			{
				if (item2.ModuleName.Equals(value)) return item2;
			}
			return null;
		}
	}
