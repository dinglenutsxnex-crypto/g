...
namespace SF3.UserData
{
	public static class RefreshBattlesProcessing
	{
		...
		{
			GD.Print(string.Format("<color=red>Try to change chapter from [{0}] to [{1}]</color>", UserManager.Instance.GetCurrentChapter(), request.Value));
		...
		private static void Response_SetChapter(NetworkEvent eventData)
		{
			if (eventData.success)
			{
				...
				GD.Print(string.Format("Successfully switched to chapter <b><color=red>[{0}]</color></b>", UserManager.Instance.GetCurrentChapter()));
			}
		}
