...
namespace SF3
{
	public partial class ModelDebugInfo : UIModuleHolder
	{
		private enum Tabs
		{
			Main = 0,
		...
		internal void _Ready()
		{
			if (NekkiUtils.IsDebug)
			{
				...
				{
					DebugInfoHider.setPlayerStatsObj = base;
				}
				else
				{
					DebugInfoHider.setEnemyStatsObj = base;
				}
		...
		private void _Process(double delta)
		{
			...
			{
				animationName = _modelState.animationInfo.AnimationPlayer.name;
				blendedAnimations = string.Empty;
				string[] playingAnimations = _modelState.modelAnimationPlayer.GetPlayingAnimations();
				...
				if (_modelState.animationIntervals != null)
				{
					...
					foreach (IntervalAnimationPlayer animationInterval in _modelState.animationIntervals)
					{
						intervalsName = intervalsName + "\n     " + animationInterval.name;
					}
				}
				animationFrame = _modelState.modelAnimationPlayer.animationKey + 1 + string.Empty;
		...
		{
			Vector3 localPosition = _bt.Position;
			localPosition.x = newPositionX;
			_bt.Position = localPosition;
		}
		private void Show()
		{
