...
using Godot;
using sf3DTO;
public partial class LocationStack
{
	...
	public void UpdatePosition()
	{
		...
		for (int i = 0; i < buttons.Count; i++)
		{
			...
			buttons[i].GlobalPosition = buttons[i].TargetAnchor.TransformPoint(offset);
		}
	}
	...
	public List<MapBattleButton> GetButtons()
	{
		...
		foreach (KeyValuePair<int, LocationStackUnit> unit in _units)
		{
			...
			if (unit.Value.Battle != null && unit.Value.Battle.activeSelf && !flag)
			{
