...
public class NetworkChain
{
	...
	public void StartChain()
	{
		...
		{
			GD.PrintErr("StartChain can only be called when timeout ended or this is the first call of the app");
		}
	}
	...
	{
		GD.Print("Stopping state: " + GetCurrentStateName());
		...
		{
			GD.PrintErr("State null was stopped. This should not happen.");
		}
		if (withTimeout)
		{
			SwitchToTimeoutState();
	...
	{
		GD.PushWarning("State Failed: " + GetCurrentStateName() + " with Error: " + failReason);
		StopCurrentState();
	}

	private void SwitchToState(Type nextStateType, object data, Action<Type, object> OnSuccessCallback = null)
	{
		GD.Print(string.Format("Switching network state: {0}->{1}", GetCurrentStateName(), nextStateType));
	...
	private static NetworkStateBase CreateStateOfType(Type type)
	{
		...
		{
			GD.PrintErr(string.Concat("Error: ", type, " is not derived from NetworkStateBase"));
		}
