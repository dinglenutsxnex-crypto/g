...
using Godot;
using sf3DTO;
public partial class ModelObjectExtended : ModelObject
{
	...
	private void Validate()
	{
		...
		if (flag)
		{
			...
			GD.PrintErr(stringWrapper.ToString());
		}
	}
	...
	private void SetVariable(int id, string varName, int value)
	{
		...
		if (variable == null)
		{
			GD.PrintErr("Variable found by key : [" + varName + "] is null");
		}
		else
		{
	...
	private void Print()
	{
		GD.PrintErr(string.Format("UsagesTotal: {0}, UsagesCurrent: {1}, RangedUsagesLeft: {2}, TimerCount: {3}", _usageCountTotal, _usageCountCurrent, (int)GameVariables.GetVariable(_id, "RangedUsagesLeft").value, _counters.Count));
	}
}
